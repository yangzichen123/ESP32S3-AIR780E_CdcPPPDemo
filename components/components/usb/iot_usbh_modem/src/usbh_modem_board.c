...
#define MODEM_POWER_GPIO_ACTIVE_MS 1500
#define MODEM_POWER_GPIO_INACTIVE_MS 1500
...
#define MODEM_STAGE_CODE2STR(code) \
    {                              \
        code, #code                \
    }
...
{
    ESP_LOGI(TAG, "Force power cat1 modem....");
    gpio_config_t io_config = {
        .pin_bit_mask = BIT64(MODEM_POWER_GPIO),
        .mode = GPIO_MODE_OUTPUT};
    gpio_config(&io_config);
    // gpio default to inactive state
    gpio_set_level(MODEM_POWER_GPIO, MODEM_POWER_GPIO_INACTIVE_LEVEL);
    // gpio active to reset modem
    gpio_set_level(MODEM_POWER_GPIO, !MODEM_POWER_GPIO_INACTIVE_LEVEL);
    ESP_LOGI(TAG, "Power cat1 modem using io=%d, level=%d", MODEM_RESET_GPIO, !MODEM_RESET_GPIO_INACTIVE_LEVEL);
    vTaskDelay(pdMS_TO_TICKS(MODEM_POWER_GPIO_ACTIVE_MS));
    gpio_set_level(MODEM_POWER_GPIO, MODEM_POWER_GPIO_INACTIVE_LEVEL);
    // waitting for modem re-init ready
    ESP_LOGI(TAG, "Waiting for cat1 modem power up ready");
...
static void _modem_daemon_task(void *param)
{
    ...
    while (true)
    {
        ...
        EventBits_t bits = xEventGroupWaitBits(s_modem_evt_hdl, (PPP_NET_MODE_ON_BIT | PPP_NET_MODE_OFF_BIT | DTE_USB_RECONNECT_BIT | DTE_USB_DISCONNECT_BIT | PPP_NET_RECONNECTING_BIT | PPP_NET_DISCONNECT_BIT | MODEM_DESTROY_BIT), pdFALSE, pdFALSE, portMAX_DELAY);
        ...
        if (bits & DTE_USB_RECONNECT_BIT)
        {
            ...
            for (size_t i = 0; i < MODEM_NET_RECONNECT_DELAY_S; i++)
            {
                ...
                if (((xEventGroupGetBits(s_modem_evt_hdl) & DTE_USB_RECONNECT_BIT) == 0) || (xEventGroupGetBits(s_modem_evt_hdl) & MODEM_DESTROY_BIT))
                {
                    break;
                }
                else
